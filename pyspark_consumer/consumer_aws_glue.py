...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - GlueJob - %(message)s',
    stream=sys.stdout
)
...
DB_NAME = args.get('DB_NAME', "postgres")
DB_USER = args.get('DB_USER')
DB_PASSWORD = args.get('DB_PASSWORD')
DB_URL = f"jdbc:postgresql://{DB_HOST}:5432/{DB_NAME}" if all([DB_HOST, DB_NAME, DB_USER, DB_PASSWORD]) else None
...
logging.info("Streaming queries iniciadas, o Job do Glue permanecerá em execução.")
